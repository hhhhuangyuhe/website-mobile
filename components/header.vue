<template>
  <div id="fyw-header">
    <img
      src="../assets/img/index/泛员云.png"
      class="fyw-logo"
      @click="$router.push({ path: '/' })"
    />
    <span v-if="!showNav" class="fyw-phone"
      ><img
        src="../assets/img/index/phone.png"
        class="phone"
      />400-0909-388</span
    >
    ...
    <img
      :src="
        showNav
          ? require('../assets/img/nav/guanbi.png')
          : require('../assets/img/index/hanbao.png')
      "
      :class="showNav ? 'guanbi' : 'hanbao'"
      @click="showNav = !showNav"
    />
    <!-- 导航 -->
    <transition
      name="fyw-nav-transition"
    ...
    >
      <div class="fyw-nav" v-show="showNav">
        ...
        <div class="fyw-nav-item" @click="showProductNav = !showProductNav">
          ...
          <span class="text" :class="showProductNav ? 'showingProducts' : ''"
            >产品与解决方案</span
          >
        ...
        <div class="fyw-nav-item product-list" v-show="showProductNav">
          <div class="products">
            <span
              class="product-item"
              :class="nowRoute == 'product-ers' ? 'active-pd' : ''"
              @click="goPath('/product/ers')"
              >员工关系服务</span
            >
            <span
              class="product-item"
              :class="nowRoute == 'product-nsso' ? 'active-pd' : ''"
              @click="goPath('/product/nsso')"
              >全国社保外包</span
            >
            <span
              class="product-item"
              :class="nowRoute == 'product-paySalary' ? 'active-pd' : ''"
              @click="goPath('/product/paySalary')"
              >薪酬核算</span
            >
            <span
              class="product-item"
              :class="nowRoute == 'product-payroll' ? 'active-pd' : ''"
              @click="goPath('/product/payroll')"
              >薪资代发</span
            >
            <span
              class="product-item"
              :class="
                nowRoute == 'product-attendanceAndVacation' ? 'active-pd' : ''
              "
              @click="goPath('/product/attendanceAndVacation')"
              >考勤与休假</span
            >
            <span
              class="product-item"
              :class="
                nowRoute == 'product-staffMedicalExamination' ? 'active-pd' : ''
              "
              @click="goPath('/product/staffMedicalExamination')"
              >员工体检</span
            >
            <span
              class="product-item"
              :class="
                nowRoute == 'product-employeeProtection' ? 'active-pd' : ''
              "
              @click="goPath('/product/employeeProtection')"
              >补充员工保障</span
            >
            <span
              class="product-item"
              :class="nowRoute == 'product-holidayBenefits' ? 'active-pd' : ''"
              @click="goPath('/product/holidayBenefits')"
              >节日福利</span
            >
            <span
              class="product-item"
              :class="nowRoute == 'product-pointsBenefit' ? 'active-pd' : ''"
              @click="goPath('/product/pointsBenefit')"
              >积分福利</span
            >
          </div>
        </div>
        <div
          class="fyw-nav-item"
          :class="nowRoute == 'information' ? 'active' : ''"
          @click="goPath('/information')"
        >
        ...
        <div
          class="fyw-nav-item"
          :class="nowRoute == 'aboutus' ? 'active' : ''"
          @click="goPath('/aboutus')"
        >
