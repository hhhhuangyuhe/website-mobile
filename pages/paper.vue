<template>
  <div id="paper">
    <div class="paper-content">
      <p class="paper-title">{{detailData._title}}</p>
      <p class="paper-time-row">
        <span class="paper-time">{{formatTime(detailData._addtime)}}</span>
        <span class="paper-nav">
          首页 >
...
export default {
  ...
  async asyncData({ app, query }) {
    let { data } = await app.$axios
      .get(`/api/News/InnerData?id=` + query.id + `&parentId=` + query.parentId)
      return { 
...
#paper .paper-content .paper-title {
  font-size: 44px;
  font-family: "思源黑体M";
  font-weight: 500;
  color: rgba(0, 0, 0, 1);
...
#paper .paper-content .paper-time-row {
  display: flex;
  align-items: center;
  justify-content: space-between;
}
#paper .paper-content .paper-time-row .paper-time {
  font-size: 24px;
  line-height: 19px;
  font-weight: 400;
  color: rgba(136, 136, 136, 1);
}
#paper .paper-content .paper-time-row .paper-nav {
  font-size: 24px;
  line-height: 22px;
  font-weight: 400;
  color: rgba(136, 136, 136, 1);
}
#paper .paper-content .paper-time-row .paper-nav .orange {
  color: #f08200;
}
#paper .paper-sammary {
  width: 670px;
...
#paper .paper-detail-words {
  font-size: 36px;
  color: rgba(0, 0, 0, 1);
  line-height: 52px;
}
#paper .paper-detail-words span {
  font-size: 36px;
  color: rgba(0, 0, 0, 1);
  line-height: 52px;
}
